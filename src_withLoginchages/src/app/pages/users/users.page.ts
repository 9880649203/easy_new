...
export class UsersPage implements OnInit {
  ...
  constructor(private modalCtrl: ModalController, private rest: RestService,
    ...
    if (this.loggedInUser.role == 'Admin') {
      this.viewAgencyManager = true;
      this.viewFieldAgent = true;
      this.viewOther = true;
    ...
    else if (this.loggedInUser.role == 'Order Manager') {
      this.viewAgencyManager = true;
      this.viewFieldAgent = true;
      this.viewOther = false;
      this.viewUser = true;
    ...
    else if (this.loggedInUser.role == 'Field Agent') {
      this.viewAgencyManager = false;
      this.viewFieldAgent = false;
      this.viewOther = false;
      this.viewUser = false;
      this.rest.roleType = null;
    }
    else if (this.loggedInUser.role == 'Viewer') {
      this.viewAgencyManager = true;
      this.viewFieldAgent = true;
      this.viewOther = true;
      this.viewUser = true;
  ...
  ngOnInit() {
    this.rest._userList$.subscribe(data => {
      if (data.length > 0) {
        ...
        console.log("üser data", this.manager_list, this.agent_list, this.other_list)
      }
      else {
        this.manager_list = []
        setTimeout(() => {
