...
export class AddOrderPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.orderForm = new FormGroup({
      ...
      ordered_forfarmer: new FormControl('', [Validators.required]),
      ordered_forexuser: new FormControl('', [Validators.required]),
      farmer_id: new FormControl('', [Validators.required]),
      on_the_behalf: new FormControl(),
      category: new FormControl('', [Validators.required]),
      product: new FormControl('', [Validators.required]),
      unit: new FormControl(),
      order_for_mobile: new FormControl('', [Validators.required, Validators.pattern('[0-9]*'), Validators.minLength(10)]),
  ...
  onItemSelect(event, t) {
    ...
    if (t === 'category') {
      ...
      if (event) {
        this.orderModal.total_price = this.orderModal.total_price + event.target.value * this.orderModal.price;
      } else {
        this.orderModal.total_price = 0;
      }
    }
  ...
  addOrder() {
    if (this.orderForm.valid) {
      ...
      this.toastr.success('success', 'Order Added Successfully');
