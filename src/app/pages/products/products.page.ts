...
export class ProductsPage {
  ...
  //     if (this.selectedItems[i].category_id === this.productName[i].category_id) {
  //       this.newcat = [];
  //       this.newcat.push(this.productName[i]);
  //       this.productName = this.newcat;
  //     }
  //   }
  //   console.log("products",this.productName);
  // }
  ...
  export() {
    ...
    this.userService._filterProductsKeys.subscribe(res =>{
      sendObjData = res;
    });
  ...
  submit(e) {
    ...
    this.userService.filterData(cid).subscribe(res => {
     const tmp: any = res;
     // console.log("category res", res);
      this.brandName = tmp.brand_name;
      this.companyName = tmp.company_name;
      this.productName = tmp.product_name;
      // if(this.productName.indexOf(tmp.product_name) == -1){
...
submitCompany(e){
    ...
    if (this.selectProductFormControl.value && this.selectProductFormControl.value.length > 0) {
      this.selectProductFormControl.value.forEach((e) => {
        if (this.selectedProductValues.indexOf(e) == -1) {
          ...
          //console.log("seleted values", this.selectedProductValues)
        }
      });
    }
  }
 ...
 //   let sendObjData ={};
  ...
  this.userService._filterProductsKeys$.next({});
  };
