...
export class FarmsdataPage {
  ...
  fieldAgentFilterData: any = []
  fieldAgentFilterIndex: any;
  ...
  ngOnInit() {
    ...
    this.rest._farmerDropDownList$.subscribe(data => {
      if (data.places || data.districts || data.agencies || data.field_agent) {
        ...
        data.field_agent = data.field_agent.filter(el => {
          return el != null;
        });
        this.dropDownPlace = [];
        ...
        this.dropDownAgent = [];
        // let agent = data.field_agent.filter(element => {
        //   console.log("agentttttttttttt1111111111", element)
        //   return element.name;
        // });
        let agent: any = []
        this.fieldAgentFilterData = [];
        this.fieldAgentFilterData = data.field_agent;
        for (let i = 0; i < data.field_agent.length; i++) {
          agent[i] = data.field_agent[i].name;
        }
        // console.log("agentttttttttttt", agent)
        this.dropDownAgent = agent
        console.log("agentttttttttttt", agent)
        this.filteredAgent = this.searchAgentControl.valueChanges
          .pipe(
            startWith<string>(''),
  ...
  */
  selectionChange(event, type, index) {
    this.fieldAgentFilterIndex = index
    console.log("filter agent", event, index)
    if (event.isUserInput && event.source.selected == false) {
      ...
      if (type == 'Field Agent') {
        let indexAgent = this.selectedAgentValues.indexOf(event.source.value);
        this.selectedAgentValues.splice(indexAgent, 1)
        let fieldAgent = this.fieldAgentFilterData[this.fieldAgentFilterIndex].mobile_no;
        let index = this.selectedDataToFilter.field_agent.indexOf(fieldAgent)
    ...
    if (event.isUserInput && event.source._selected) {
      if (type == 'District') {
        ...
        let fieldAgent = this.fieldAgentFilterData[this.fieldAgentFilterIndex].mobile_no;
        this.selectedDataToFilter.field_agent = this.selectedDataToFilter.field_agent.concat(fieldAgent)
      }
  ...
  openedChangeAgent(e) {
    ...
    this.searchAgentControl.patchValue('');
    if (e == true) {
      this.searchTextBox.nativeElement.focus();
    }
  }
  ...
  remove(filter): void {
    ...
    if (filter.type == 'Field Agent') {
      let indexAgent = this.selectedAgentValues.indexOf(filter.displayName);
      this.selectedAgentValues.splice(indexAgent, 1)
      let index = this.selectedDataToFilter.field_agent.indexOf(filter.displayName)
      this.selectedDataToFilter.field_agent.splice(index, 1)
    }
