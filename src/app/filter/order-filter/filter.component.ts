...
export class FilterComponent implements OnInit {
  ...
  selectedDataToFilter: any = {
    district: [],
    place: [], agency_name: [],
    field_agent: [], category_name: [], product_name: [],
    start_date: '', end_date: ''
  };
  ...
  constructor(private fb: FormBuilder, private http: HttpClient,
    ...
    this.orderService.getProducts().subscribe(res => {
     ...
     // console.log('districtfilter', filtered);
      ...
      this.place = res.place.filter(el => {
        return el != null;
      });
      // agency
      let tmpagency = [];
      res.agency_name.forEach(an => {
        if (an) {
          tmpagency.push(an.agency_name);
        }
      });
      this.orderService._initializeAgency$.next(tmpagency);
      // console.log('tmpagency', tmpagency);
      // category
      let temp = [];
      ...
      res.field_agent.forEach(fan => {
        if (fan) {
          tempfieldagent.push(fan.order_by);
          this.fieldAgentMobiles.push(fan);
        }
      });
  ...
  private _filterFieldAgent(name: string): String[] {
    ...
    // console.log("this.filteredFieldAgent", filteredList);
    return filteredList;
  }
  ...
  selectionChange(event, type) {
    ...
    if (event.isUserInput && event.source.selected === false) {
      ...
      if (type === 'Field_agent') {
        ...
        const index = this.selectedDataToFilter.field_agent.indexOf(event.source.value);
        this.selectedDataToFilter.field_agent.splice(index, 1);
      }
  ...
  setSelectedValuesCategory() {
    // console.log('setSelectedValuesCategory', this.selectCategoryFormControl)
  ...
  remove(filter): void {
    ...
    if (filter.type === 'Agency') {
      ...
      this.selectedFieldAgentValues.splice(indexFieldAgent, 1);
      index = this.selectedDataToFilter.field_agent.indexOf(filter.displayName);
      this.selectedDataToFilter.field_agent.splice(index, 1);
    }
  ...
  clearFilter() {
    ...
    this.indentList();
    this.orderService._filterOrderKeys$.next({});
  ...
  indentList() {
    ...
    this.http
      ...
      .then((res: any) => {
        console.log('ooo',res) ;
        this.orderService._totalCount$.next(res.order_count);
        this.orderService._order_list$.next(res.message);
      })
      .then((data: any) => console.log('ooo1',data) );
  }
  ...
  submitFilter() {
    ...
    if (finalObject['field_agent'].length) {
      let tmp = [];
      finalObject['field_agent'].forEach(res => {

        this.fieldAgentMobiles.forEach(t => {
          if (t.order_by == res) {
            tmp.push(t.order_by_id);
          }
        });
      });

      finalObject['field_agent'] = tmp;
    }
    ...
    Object.keys(finalObject).forEach(key => {
      if (Array.isArray(finalObject[key])) {
        ...
        (finalObject[key] == "" || finalObject[key] === null) ? delete finalObject[key] : key;
      }
    });
    // console.log('this.fieldAgentMobiles111', this.fieldAgentMobiles);
    this.orderService._filterOrderKeys$.next(finalObject);
    this.orderService.filterOrder(finalObject);
    this.orderService._clear_order$.next('clear filter');
  }
